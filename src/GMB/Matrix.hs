...
requestThenViewReply :: (MonadHttp f,Functor f,ToJSON input,FromJSON output) => Text.Text -> input -> HttpMethod -> f output
requestThenViewReply url request method = viewReply <$> (jsonHttpRequest (HttpRequest url method "application/json" request))
