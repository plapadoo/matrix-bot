...
data IncomingMessage = IncomingMessage (Text.Text,Maybe Text.Text) deriving(Eq,Show)
  ...
  case breakOnMaybe bodyStart mb of
    ...
    Just (_,bodyText) ->
      case breakOnMaybe bodyEnd bodyText of
        Nothing -> IncomingMessage (mb,Nothing)
        Just (bodyTextTillEnd,bodyEnd) -> IncomingMessage (Text.drop 7 bodyEnd,Just (Text.drop 6 bodyTextTillEnd))
...
coparseIncomingMessage (IncomingMessage (plain,markup)) = (foldMap bodySurround markup) <> plain
