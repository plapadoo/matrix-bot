...
import           GMB.IncomingMessage                  (IncomingMessage, bodyEnd,
                                                       bodyStart,
...
import           GMB.Matrix                           (MatrixJoinReply (..),
                                                       MatrixLoginReply (..),
                                                       MatrixSendMessageReply (..),
                                                       MonadMatrix (..))
import           GMB.MonadLog                         (MonadLog (..))
import           GMB.Util                             (none, textShow)
import           GMB.WebServer                        (WebServerInput (..),
                                                       WebServerOutput (..),
                                                       handleMessage)
...
data MockOperations = OperationPutLog Text
...
processorsNoError = MatrixProcessors loginNoError joinNoError sendMessageNoError
...
instance MonadMatrix MyMonad where
  login mc request = tell [OperationMatrixLogin request] *> view mpLogin <*> pure request
  joinRoom mc request = tell [OperationMatrixJoin request] *> view mpJoin <*> pure request
  sendMessage mc request = tell [OperationMatrixSendMessage request] *> view mpSendMessage <*> pure request

case_joinFails = do
  let (WebServerOutput _ status,logs) = (runWriter . (flip runReaderT) (processorsNoError & mpJoin .~ (const (MatrixJoinReply (Just "foo") (Just "bar")))) . runMyMonad) ( handleMessage (WebServerInput (MatrixContext "baseurl") "accesstoken" "room" "<body>markup</body>simple") )
  mapM_ (putStrLn . textShow) logs
  assertBool "No join found! " (any (has _OperationMatrixJoin) logs)
  assertBool "Send message found! " (none (has _OperationMatrixSendMessage) logs)
  forbidden403 @=? status

case_allSucceeds = do
  let (WebServerOutput _ status,logs) = (runWriter . (flip runReaderT) processorsNoError . runMyMonad) ( handleMessage (WebServerInput (MatrixContext "baseurl") "accesstoken" "room" "<body>markup</body>simple") )
  mapM_ (putStrLn . textShow) logs
  assertBool "No join found! " (any (has _OperationMatrixJoin) logs)
  assertBool "No send message found! " (any (has _OperationMatrixSendMessage) logs)
  ok200 @=? status
